...
var stream = require("nu-stream")["stream"],
    ...
    doWhileStatement = (function(body, test) {
        return statement(keyword("do"), body, keyword("while"), punctuator("("), test, punctuator(")"), punctuator(
            ";"));
    }),
    ...
    forInStatement = (function(left, right) {
        return statement(keyword("for"), punctuator("("), node.left, punctuator("in"), node.right, punctuator(")"),
            node.body);
    }),
    ...
    _unparse = (function(node) {
        ...
        switch (node.type) {
            ...
            case "IfStatement":
                return ifStatement(_unparse(node.test), _unparse(node.consequent), (node.alternate ? _unparse(node.alternate) :
                    null));
            ...
            case "TryStatement":
                return tryStatement(_unparse(node.block), (node.handler ? _unparse(node.handler) : null), (node.finalizer ?
                    _unparse(node.finalizer) : null));
            case "WhileStatement":
            ...
            case "ForStatement":
                ...
                return ((node.init && (node.init.type === "VariableDeclaration")) ? forDeclarationStatement(init,
                    test, update, body) : forStatement(init, test, update, body));
            case "ForInStatement":
