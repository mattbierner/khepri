 ...
 * DO NOT EDIT
*/
define(["require", "exports", "path", "fs", "./compile"], (function(require, exports, path, fs, __o) {
    "use strict";
    var compile = __o["compile"],
            ...
            if (fs.lstatSync(resolvedPath)
                ...
                files.forEach((function(file) {
                    ...
                    if (fs.lstatSync(subPath)
                        .isDirectory()) {
                        return compileFile(subPath, (outFile && path.join(
                            outFile, file)), header, options, ok, error);
                    }
                    if (path.extname(file)
                        .match(KHEPRI_EXT)) return compileFile(subPath, (
                        outFile && path.join(outFile, (path.basename(file,
                            ".kep") + ".js"))), header, options, ok, error);
                }));
            }));
            fs.readFile(resolvedPath, "utf8", (function(err, data) {
                if (err) throw err;
                if (outFile) process.stdout.write((((("Khepri'" + inFile) + "' to:'") +
                    outFile) + "'"));
