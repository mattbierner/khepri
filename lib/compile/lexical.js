    ...
    var _check = function(node) {
        ...
        switch(node.type){
            ...
            return block(next(seq(node.bindings.map(function(x) {
                return addImmutableBinding(x.id.name, x.id.loc.start);
            }
            )), _check(node.body)));
            case "FunctionExpression":
            return block(seq([(node.id ? addImmutableBindingInRealBlock(node.id.name, node.loc) : ok()), seq(map(node.params, function(x) {
