...
define((function(require, exports, module) {
    "use strict";
    var check;
    var ast_node = require("khepri_ast/node");
    var ast_pattern = require("khepri_ast/pattern");
    var ast_value = require("khepri_ast/value");
    ...
    var emptyBlock = (function(body) {
        return examineScope((function(s) {
            return seq([setScope(new Scope(({}), null, ({}))), body, setScope(s)]);
        }));
    });
    var realBlock = (function(body) {
        return examineRealScope((function(s) {
            return seq([setRealScope(new Scope(({}), s, ({}))), body, setRealScope(s)]);
        }));
    });
    ...
    var addMapping = (function(id, newId) {
        return modifyRealScope((function(s) {
    ...
    var addUniqueMutableBinding = (function(id, loc) {
        return examineRealScope((function(s) {
            return (s.hasOwnBinding(id) ? (function(new_id) {
                return next(addMutableBinding(new_id, loc), addMapping(id, new_id));
    ...
    var addUniqueImmutableBinding = (function(id, loc) {
        return seq([checkCanAddOwnBinding(id, loc), examineScope((function(s) {
            return (s.hasBinding(id) ? (function(new_id) {
                return next(addImmutableBinding(new_id, loc), addMapping(id, new_id));
    ...
    var _check = (function(node) {
        ...
        switch (node.type) {
            ...
            case "WithStatement":
                return block(next(seq(node.bindings.map(_check)), _check(node.body)));
            ...
            case "FunctionExpression":
                return realBlock(seq([(node.id ? addUniqueImmutableBinding(node.id.name, node.loc) : ok()), _check(node.params), seq(map(node.body.body, _check))]));
            case "Program":
                return (Array.isArray(node.body) ? seq(map(node.body, _check)) : _check(node.body));
            case "Package":
                return seq([addImmutableBindingInRealBlock("require", null), addImmutableBindingInRealBlock("exports", null), addImmutableBindingInRealBlock("module", null), _check(node.exports), _check(node.imports), _check(node.body)]);
            case "PackageExports":
                return seq(map(node.exports, _check));
            case "PackageExport":
                return addMutableBinding(node.id.name, node.loc);
            case "PackageImports":
                return seq(map(node.imports, _check));
            case "PackageImport":
            ...
            case "Binding":
                return seq([_check(node.pattern), _check(node.value)]);
            case "IdentifierPattern":
                return seq([addUniqueImmutableBinding(node.name, node.loc), _check(node.init), examineRealScope((function(s) {
            ...
            case "ArrayPattern":
                return examineScope((function(s) {
                    if (!node.id) {
                        var id = ast_pattern.IdentifierPattern.create(null, s.getUnusedId("__a"));
                        (id.gen = true);
                        (node.id = id);
                    }

                    return seq([_check(node.id), seq(map(node.elements, _check))]);
                }));
            case "ObjectPattern":
                return examineScope((function(s) {
                    if (!node.id) {
                    ...
                    return seq([_check(node.id), seq(map(node.elements, (function(x) {
                        return (x.value ? _check(x.value) : addImmutableBindingInRealBlock(x.key.name, null));
                    })))]);
                }));
            case "EllipsisPattern":
                return ok();
            case "ParameterList":
                return seq(map(node.patterns, _check));
            case "Identifier":
                return examineRealScope((function(s) {
                    if (s.hasMapping(node.name))(node.name = s.getMapping(node.name));
