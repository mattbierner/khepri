    ...
    var _check = (function(node) {
        ...
        switch (node.type) {
            ...
            case "IdentifierPattern":
                return seq([tryAddMutableBinding(node.name, node.loc), _check(node.init), examine((function(s) {
            ...
            case "ArrayPattern":
                return examine((function(s) {
                    if (!node.id) {
                        var id = ast_value.Identifier.create(null, s.getUnusedId("__a"));
                        (id.gen = true);
                        (node.id = id);
                    }

                    return seq([addImmutableBindingInRealBlock(node.id.name, node.loc), seq(map(node.elements, _check))]);
                }));
            case "ObjectPattern":
                return examine((function(s) {
                    if (!node.id) {
                    ...
                    return seq([addImmutableBindingInRealBlock(node.id.name, node.loc), seq(map(node.elements, (function(x) {
