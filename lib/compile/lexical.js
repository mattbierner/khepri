    ...
    var _check = (function(node) {
        ...
        switch (node.type) {
            ...
            case "ObjectPattern":
                return examineScope((function(s) {
                    if (!node.id) {
                        var id = ast_pattern.IdentifierPattern.create(null, s.getUnusedId("__o"));
                        (id.gen = true);
                        (node.id = id);
                    }

                    (node.elements = map(node.elements, (function(x) {
                        return (x.value ? x : ({
                            "key": ast_value.Literal.create(null, "string", x.key.name),
                            "value": x.key
                        }));
                    })));
                    return next(_check(node.id), seq(map(node.elements, (function(x) {
                        return _check(x.value);
                    }))));
                }));
