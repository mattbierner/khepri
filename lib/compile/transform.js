...
define(["khepri_ast/clause", "khepri_ast/declaration", "khepri_ast/expression", "khepri_ast/program", "khepri_ast/statement", "khepri_ast/value"], function(ast_clause, ast_declaration, ast_expression, ast_program, ast_statement, ast_value) {
    ...
    var arrayPattern = function(base, pattern, index) {
        switch(pattern.type){
            ...
            return ast_declaration.VariableDeclarator.create(null, ast_value.Identifier.create(null, pattern.name), ast_expression.MemberExpression.create(null, base, ast_value.Literal.create(null, "number", index), true));
            case "ArrayPattern":
            var innerBase = ast_expression.MemberExpression.create(null, base, ast_value.Literal.create(null, "number", index), true);
            return concat([ast_declaration.VariableDeclarator.create(null, ast_value.Identifier.create(null, pattern.name), ast_expression.MemberExpression.create(null, base, ast_value.Literal.create(null, "number", index), true))], map(pattern.elements, function(x, i) {
    ...
    var transform = function(node) {
        ...
        switch(node.type){
            ...
            (params = reduce(params, function(p, c) {
                if ((c.type === "EllipsisPattern"))return p;
