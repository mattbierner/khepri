...
define(["ecma_ast/clause", "ecma_ast/declaration", "ecma_ast/expression", "ecma_ast/node", "ecma_ast/program", "ecma_ast/statement", "ecma_ast/value", "khepri_ast/clause", "khepri_ast/declaration", "khepri_ast/expression", "khepri_ast/node", "khepri_ast/pattern", "khepri_ast/program", "khepri_ast/statement", "khepri_ast/value"], (function(ecma_clause, ecma_declaration, ecma_expression, ecma_node, ecma_program, ecma_statement, ecma_value, khepri_clause, khepri_declaration, khepri_expression, khepri_node, khepri_pattern, khepri_program, khepri_statement, khepri_value) {
    ...
    var transform = (function(node) {
        ...
        switch (node.type) {
                    ...
                    if ((imp.type === "ImportPattern")) {
                        ...
                        switch (imp.pattern.type) {
                            ...
                            case "ObjectPattern":
                                ...
                                return concat(p, ecma_declaration.VariableDeclarator.create(null, transform(imp.pattern.id), base), reduce(imp.pattern.elements, (function(p, c) {
                                    return (function(val) {
                                        return (val ? concat(p, val) : p);
            ...
            case "Package":
                var imports = ((node.body.type === "WithStatement") ? map(node.body.bindings.filter((function(x) {
                ...
                var fBody = ((node.body.type === "WithStatement") ? transform(khepri_statement.WithStatement.create(null, node.body.bindings.map((function(x) {
                    return ((x.type !== "ImportPattern") ? x : khepri_declaration.Binding.create(null, x.pattern, imports.map((function(x) {
                        return ecma_value.Identifier.create(null, x.from.value);
                    }))));
                })), node.body.body)) : transform(node.body));
                var body = ecma_expression.FunctionExpression.create(null, null, concat(ecma_value.Identifier.create(null, "require"), ecma_value.Identifier.create(null, "exports"), imports.map((function(x) {
                    return ecma_value.Identifier.create(null, x.value);
