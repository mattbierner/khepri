...
define(["ecma_ast/clause", "ecma_ast/declaration", "ecma_ast/expression", "ecma_ast/node", "ecma_ast/program", "ecma_ast/statement", "ecma_ast/value", "khepri_ast/clause", "khepri_ast/declaration", "khepri_ast/expression", "khepri_ast/node", "khepri_ast/pattern", "khepri_ast/program", "khepri_ast/statement", "khepri_ast/value"], (function(ecma_clause, ecma_declaration, ecma_expression, ecma_node, ecma_program, ecma_statement, ecma_value, khepri_clause, khepri_declaration, khepri_expression, khepri_node, khepri_pattern, khepri_program, khepri_statement, khepri_value) {
    ...
    var objectPattern = (function(base, pattern, key, f) {
        var k = ((key.type === "IdentifierPattern") ? khepri_value.Literal.create(null, "string", key.name) : key);
        ...
        switch (pattern.type) {
            case "IdentifierPattern":
                return f(khepri_value.Identifier.create(null, pattern.name), innerBase);
            case "ArrayPattern":
            ...
            case "ObjectPattern":
                var elements = reduce(pattern.elements, (function(p, c) {
                    return (function(val) {
                        return (val ? concat(p, val) : p);
                    })(objectPattern(innerBase, c.value, c.key));
                }), []);
                return (pattern.id.gen ? elements : concat([f(pattern.id, innerBase)], elements));
            default:
                return null;
    ...
    var objectDeclPattern = (function(make) {
        return (function(base, pattern, key) {
            return objectPattern(base, pattern, key, make);
        });
    })((function make(l, r) {
        return ecma_declaration.VariableDeclarator.create(null, l, r);
    }));
    var objectAssignmentPattern = (function(make) {
        return (function(base, pattern, key) {
            return objectPattern(base, pattern, key, make);
        });
    })((function make(l, r) {
    ...
    var transform = (function(node) {
        ...
        switch (node.type) {
            ...
            case "WithStatement":
                var importBody = node.bindings.map((function(imp) {
                    var base = imp.value;
                    switch (imp.pattern.type) {
                        case "IdentifierPattern":
                            return ecma_declaration.VariableDeclaration.create(null, [khepri_declaration.VariableDeclarator.create(null, transform(imp.pattern), base)]);
                        case "ObjectPattern":
                            var b = khepri_value.Identifier.create(null, imp.pattern.id.name);
                            return ecma_declaration.VariableDeclaration.create(null, concat(ecma_declaration.VariableDeclarator.create(null, transform(imp.pattern.id), base), reduce(imp.pattern.elements, (function(p, c) {
                                return (function(val) {
                                    return (val ? concat(p, val) : p);
                                })(objectDeclPattern(transform(imp.pattern.id), c.value, c.key));
                            }), [])));
                    }
                    return imp;
                }));
                var fBody = transform(node.body);
                return ecma_statement.BlockStatement.create(fBody.loc, concat(importBody, fBody.body));
            ...
            case "LetExpression":
                return node.bindings.reduceRight((function(p, c) {
                    return new khepri_expression.CallExpression(null, transform(new khepri_expression.FunctionExpression(null, null, khepri_pattern.ParameterList.create(null, [c.pattern]), transform(new khepri_statement.BlockStatement(null, [new khepri_statement.ReturnStatement(null, p)])))), [((c.value.type === "FunctionExpression") ? new ecma_expression.FunctionExpression(null, (c.value.id || ((c.pattern.type === "IdentifierPattern") ? ecma_value.Identifier.create(null, c.pattern.name) : null)), transform(c.value.params), transform(c.value.body)) : transform(c.value))]);
                }), transform(node.body));
                    ...
                    switch (c.type) {
                        ...
                        case "ObjectPattern":
                            ...
                            return concat(p, khepri_declaration.VariableDeclaration.create(null, reduce(c.elements, (function(p, c) {
                                return (function(val) {
                                    return (val ? concat(p, val) : p);
                                })(objectDeclPattern(base, c.value, c.key));
                            }), [])));
                    }
                    return p;
                }), []);
            ...
            case "EllipsisPattern":
                return null;
            case "Program":
                return new ecma_program.Program(node.loc, (Array.isArray(node.body) ? map(node.body, transform) : [transform(node.body)]));
            case "Package":
                var imports = (node.imports ? map(node.imports.imports, (function(x) {
                    return x.from;
                })) : []);
                var importBody = (node.imports ? node.imports.imports : []).map((function(imp) {
                    var base = ecma_expression.CallExpression.create(null, ecma_value.Identifier.create(null, "require"), [imp.from]);
                    switch (imp.pattern.type) {
                        case "IdentifierPattern":
                            return ecma_declaration.VariableDeclaration.create(null, [khepri_declaration.VariableDeclarator.create(null, transform(imp.pattern), base)]);
                        case "ObjectPattern":
                ...
                var fBody = transform(node.body);
                var body = ecma_expression.FunctionExpression.create(null, null, [ecma_value.Identifier.create(null, "require"), ecma_value.Identifier.create(null, "exports"), ecma_value.Identifier.create(null, "module")], ecma_statement.BlockStatement.create(fBody.loc, concat(ecma_statement.ExpressionStatement.create(null, ecma_value.Literal.create(null, "string", "use strict")), exportHeader, importBody, fBody.body, exportBody)));
                return ecma_expression.CallExpression.create(node.loc, ecma_value.Identifier.create(null, "define"), [body]);
            case "PackageBody":
