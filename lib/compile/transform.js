...
define(["ecma/ast/clause", "ecma/ast/declaration", "ecma/ast/expression", "ecma/ast/program", "ecma/ast/statement", "ecma/ast/value", "khepri/ast/khepri"], function(astClause, astDeclaration, astExpression, astProgram, astStatement, astValue, astKhepri) {
    ...
    var transform = function(node) {
        ...
        switch(node.type){
            ...
            return new astStatement.ExpressionStatement(node.loc, new astExpression.AssignmentExpression(node.loc, node.operator, transform(node.left), transform(node.right)));
            ...
            return new astExpression.Logical(node.loc, node.operator, transform(node.left), transform(node.right));
            ...
            case "LetExpression":
            return node.bindings.reduce(function(p, c) {
                return new astExpression.CallExpression(null, new astExpression.FunctionExpression(null, null, [c.id], new astStatement.BlockStatement(null, [new astStatement.ReturnStatement(null, p)])), [c.init]);
            }
            , transform(node.body));
