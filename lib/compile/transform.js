    ...
    var transform = function(node) {
        ...
        switch(node.type){
            ...
            return node.bindings.reduceRight(function(p, c) {
                return new astExpression.CallExpression(null, new astExpression.FunctionExpression(null, null, [c.id], new astStatement.BlockStatement(null, [new astStatement.ReturnStatement(null, p)])), [((c.init.type === "FunctionExpression") ? new astExpression.FunctionExpression(null, (c.init.id || c.id), transform(c.init.params), transform(c.init.body)) : transform(c.init))]);
            }
            , transform(node.body));
            case "FunctionExpression":
            return new astExpression.FunctionExpression(node.loc, transform(node.id), transform(node.params), transform(node.body));
            case "ParameterList":
            return map(reduce(node.patterns, function(p, c) {
