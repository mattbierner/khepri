            ...
            case "WithStatement":
                var importBody = transform(khepri_declaration.VariableDeclaration.create(null, flatten(node.bindings.map((function(imp) {
                    ...
                    switch (imp.pattern.type) {
                        case "IdentifierPattern":
                            return khepri_declaration.VariableDeclarator.create(null, imp.pattern, base);
                        case "ObjectPattern":
                            return concat(khepri_declaration.VariableDeclarator.create(null, imp.pattern.id, base), reduce(imp.pattern.elements, (function(p, c) {
                                return (function(val) {
                                    return (val ? concat(p, val) : p);
                                })(objectDeclPattern(imp.pattern.id, c.value, c.key));
                            }), []));
                        case "ArrayPattern":
                            return concat(khepri_declaration.VariableDeclarator.create(null, imp.pattern.id, base), reduce(imp.pattern.elements, (function(p, c, i) {
                                return (function(val) {
                                    return (val ? concat(p, val) : p);
