 ...
 */
define(['parse/parse', 'parse/parse_string', 'parse/parse_eager', 'stream', 'ecma/lex/line_terminator_lexer'],
    function(parse, parse_string, parse_eager, stream, line_terminator){
//"use strict";
...
var singleLineComment = parse.next(singleLineCommentMarker,
    parse.bind(parse_eager.many(singleLineCommentChar), function(chars) {
 ...
 */
var multiLineCommentChars = parse.NamedRecParser('multiLineCommentChars Lexer', function(self) {
    return parse.either(
...
var multiLineComment = parse.next(multiLineCommentStartMarker,
    parse.bind(multiLineCommentChars, function(chars) {
        return parse.always(stream.toArray(chars).join(''));
    }));
