 ...
 */
define(['parse/parse', 'parse/parse_string', 'stream', 'ecma/lex/reserved_word_lexer', 'ecma/lex/string_lexer'],
function(parse, parse_string, stream, reserved_word_lexer, string_lexer){
"use strict";
...
var identifierName = parse.bind(identifierStart, function(start) {
    return parse.bind(identifierParts, function(rest) {
        return parse.always(stream.cons(start, rest));
    });
});

// Identifier
////////////////////////////////////////
var _reservedWordTest = parse.next(reserved_word_lexer.reservedWord, parse.eof());
...
var identifier = parse.Parser('Identifier Lexer',
    parse.bind(identifierName, function(name) {
        return (parse.testStream(_reservedWordTest, name) ?
            parse.never() :
            parse.always(stream.toArray(name).join('')));
    }));
