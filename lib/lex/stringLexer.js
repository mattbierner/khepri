 ...
 */
define(['parse', 'lineTerminatorLexer', 'numberLexer'],
function(parse, lineTerminatorLexer, numberLexer){
"use strict";

/* Helpers
 ******************************************************************************/
var fromCharCodeLexer = function(digits) {
...
var lineContinuation = parse.next(
    parse.next(escape, lineTerminatorLexer.lineTerminatorSequence),
    parse.always(''));
...
var nonEscapeCharacter = parse.token(function(tok) {
    return !(parse.test(escapeCharacter, tok) ||
        parse.test(lineTerminatorLexer.lineTerminator, tok));
});
...
var hexEscapeSequence = parse.next(parse.character('x'),
    parse.bind(parse.times(2, numberLexer.hexDigit), fromCharCodeLexer));
...
var unicodeEscapeSequence = parse.next(parse.character('u'),
    parse.bind(parse.times(4, numberLexer.hexDigit), fromCharCodeLexer));
...
var escapeSequence = parse.choice(
    ...
    parse.next(parse.character('0'), parse.next(parse.either(
        ...
        parse.token(function(tok) {
            return !parse.test(numberLexer.decimalDigit, tok);
        })
    ), parse.always('\u0000'))),
    hexEscapeSequence,
