...
// Literal
////////////////////////////////////////
var nullLiteral = parse.bind(token.nullLiteral, function(x, state) {
    return parse.always(new state.ast.value.Literal(x.value, 'null'));
});

var booleanLiteral = parse.bind(token.booleanLiteral, function(x, state) {
    return parse.always(new state.ast.value.Literal(x.value, 'boolean'));
});

var numericLiteral = parse.bind(token.numericLiteral, function(x, state) {
    return parse.always(new state.ast.value.Literal(x.value, 'number'));
});

var stringLiteral = parse.bind(token.stringLiteral, function(x, state) {
    return parse.always(new state.ast.value.Literal(x.value, 'string'));
});

var regularExpressionLiteral = parse.bind(token.regularExpressionLiteral, function(x, state) {
...
var primaryExpression = parse.Parser('Primary Expression',
    parse.choice(
        parse.bind(
            token.keyword('this'),
            function(_, state) {
                return parse.always(new state.ast.expression.ThisExpression());
            }),
        token.anyIdentifier,
        literal,
        arrayLiteral,
    ...
    return parse.binda(
            ...
            function(expression, props, state) {
                return parse.always(stream.reduce(props, function(p, c){
                    return new state.ast.expression.MemberExpression(p, c.property, c.computed);
                }, expression));
            });
});
        ...
        function(member, a, accessors, state) {
            return parse.always(stream.reduce(accessors, function(p, c) {
                return (c.hasOwnProperty('property') ?
                     new state.ast.expression.MemberExpression(p, c.property, c.computed) :
        ...
        function(ops, expression, state) {
            return parse.always(stream.reduceRight(ops, function(argument, op) {
                return (op.value === '++' || op.value === '--' ?
                    ...
                    new state.ast.expression.UnaryExpression(op.value, argument));
            }, expression));
        });
}());
    ...
    return parse.backtrack(parse.either(
        parse.attempt(parse.binda(
            ...
            function(left, operator, right, state) {
                return parse.always(new state.ast.expression.AssignmentExpression(operator.value, left, right));
            })),
