 ...
 */
define(['require', 'program', 'parse', 'ast', 'token'],
...
var functionExpression = function() {
    return require('program').functionExpression.apply(undefined, arguments);
};
...
var postfixExpression = parse.bind(
    parse.sequence(
        leftHandSideExpression,
        parse.optional(postfixOperator)),
    function(seq) {
        return (seq[1].length > 0 ?
            parse.always(new PostfixExpressionNode(seq[1], seq[0])) :
            parse.always(seq[0]));
    });
...
var unaryExpression = parse.bind(
    parse.sequence(
        parse.many(unaryOperator),
        postfixExpression),
    function(seq) {
        return parse.always(seq[0].reduceRight(function(p, c) {
