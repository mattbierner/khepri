...
define(["require", "exports", "parse/parse", "parse/lang", "nu/stream", "khepri_ast/declaration", "khepri_ast/expression", "khepri_ast/statement", "khepri_ast/pattern", "khepri/position", "khepri/parse/common", "khepri/parse/token_parser", "khepri/parse/program_parser", "khepri/parse/value_parser", "khepri/parse/pattern_parser"], (function(require, exports, __o, __o0, __o1, ast_declaration, ast_expression, ast_statement, ast_pattern, __o2, __o3, __o4, _, __o5, pattern) {
    ...
    var __o = __o,
        ...
        pattern = pattern; {
            ...
            (propertyInitializer = Parser("Property Initializer", nodea(enumeration(propertyName, next(punctuator(":"), expression)), (function(loc, name, value) {
                return ({
                    "loc": loc,
                    "key": name,
                    "kind": "init",
            ...
            var letBinding = Parser("Let Binding", nodea(enumeration(then(letIdentifier, punctuator("=")), expected("expression", expression)), ast_declaration.Binding.create));
            ...
            (newExpression = Parser("New Expression", nodea(next(keyword("new"), enumeration(expected("member expression", memberExpression), optional([], args))), ast_expression.NewExpression.create)));
            (leftHandSideExpression = (function() {
                {
                    var reducer = (function(p, c) {
                        ...
                        return Parser("Left Hand Side Expression", binds(enumeration(memo(memberExpression), many(either(args, accessor))), (function(member, accessors) {
            ...
            (leftHandReferenceExpression = (function() {
                {
                    var reducer = (function(p, c) {
                        ...
                        return Parser("Left Hand Reference Expression", binds(enumeration(choice(thisExpression, identifier), many(accessor)), (function(expression, props) {
                            return always(foldl(reducer, expression, props));
                        })));
                    }
                }
            ...
            (conditionalExpression = (function() {
                {
                    var conditionalBody = next(punctuator("?"), enumeration(expected("expression", expression), next(punctuator(":"), expected("expression", expression)))); {
                        return bind(binaryExpression, (function(bin) {
                            return optional(bin, nodea(conditionalBody, (function(loc, consequent, alternate) {
            ...
            var rightComposeOperator = punctuator("\\>", "\\>>");
            var leftComposeOperator = punctuator("<\\", "<<\\");
            var leftPipeOperator = punctuator("|>");
            var rightPipeOperator = punctuator("<|");
            var composePrecedenceTable = [({
                "sep": rightComposeOperator,
                "precedence": 1,
                "node": ast_expression.BinaryExpression
            }), ({
                "sep": leftComposeOperator,
                "precedence": 1,
                "right": true,
                "node": ast_expression.BinaryExpression
            }), ({
                "sep": leftPipeOperator,
                "precedence": 2,
                "node": ast_expression.BinaryExpression
            }), ({
                "sep": rightPipeOperator,
                "precedence": 2,
                "right": true,
                "node": ast_expression.BinaryExpression
            })];
            (composeExpression = Parser("Compose Expression", precedence(conditionalExpression, composePrecedenceTable)));
            (assignmentOperator = Parser("Assignment Operator", punctuator("=")));
            (assignmentExpression = Parser("Assignment Expression", nodea(append(attempt(enumeration(leftHandReferenceExpression, assignmentOperator)), enumeration(expected("expression", expression))), (function(loc, left, op, right) {
            ...
            var deleteExpression = (function() {
                {
                    var reducer = (function(argument, op) {
                        ...
                        return Parser("Delete Expression", nodea(enumeration(deleteOperator, expected("left hand side expression", leftHandSideExpression)), (function(loc, op, expression) {
