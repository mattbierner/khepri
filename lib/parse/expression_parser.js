...
*/
;
define(["require", "parse/parse", "parse/lang", "nu/stream", "ecma/position", "ecma/parse/common", "ecma/parse/token_parser", "khepri_ast/declaration", "khepri_ast/expression", "khepri_ast/statement", "khepri_ast/pattern", "khepri/parse/program_parser", "khepri/parse/value_parser", "khepri/parse/pattern_parser"], (function(require, parse, parse_lang, stream, position, ecma_parse, token, ast_declaration, ast_expression, ast_statement, ast_pattern, program, value, pattern) {
    "use strict";
    ...
    var propertyValueInitializer = parse.Parser("Property Value Initializer", ecma_parse.nodea(parse.enumeration(propertyName, token.punctuator(":"), expression), (function(loc, name, _, value) {
        return ({
            "loc": loc,
            "key": name,
            "kind": "init",
    ...
    var propertyGetInitializer = parse.Parser("Property Get Initializer", ecma_parse.nodea(parse.next(token.keyword("get"), parse.enumeration(propertyName, token.punctuator("("), token.punctuator(")"), parse_lang.between(token.punctuator("{"), token.punctuator("}"), functionBody))), (function(loc, name, _0, _1, body) {
        return ({
            "loc": loc,
            "key": name,
            "kind": "get",
    ...
    var propertySetInitializer = parse.Parser("Property Set Initializer", ecma_parse.nodea(parse.next(token.keyword("set"), parse.enumeration(propertyName, parse_lang.between(token.punctuator("("), token.punctuator(")"), propertySetParameterList), parse_lang.between(token.punctuator("{"), token.punctuator("}"), functionBody))), (function(loc, name, parameters, body) {
        return ({
            "loc": loc,
            "key": name,
            "kind": "set",
    ...
    var ecmaFunctionExpression = ecma_parse.nodea(parse.next(token.keyword("function"), parse.enumeration(parse.optional(null, value.identifier), parse_lang.between(token.punctuator("("), token.punctuator(")"), formalParameterList), parse_lang.between(token.punctuator("{"), token.punctuator("}"), functionBody))), ast_expression.FunctionExpression.create);
    ...
    var lambdaFunctionExpression = ecma_parse.nodea(parse.next(token.punctuator("\\"), parse.enumeration(lambdaFormalParameterList, token.punctuator("->"), lambdaBody)), (function(loc, parameters, _, body) {
    ...
    var letBinding = parse.Parser("Let Binding", ecma_parse.nodea(parse.enumeration(parse_lang.then(letIdentifier, token.punctuator("=")), conditionalExpression), ast_declaration.Binding.create));
    var letExpression = parse.Parser("Let Expression", ecma_parse.nodea(parse.next(token.keyword("let"), parse.enumeration(parse.eager(parse_lang.sepBy1(token.punctuator(","), letBinding)), parse.next(token.keyword("in"), conditionalExpression))), ast_expression.LetExpression.create));
    ...
    var memberExpression = (function() {
        {
            var reducer = (function(p, c) {
                ...
                return parse.Parser("Member Expression", parse.binds(parse.enumeration(parse.either(newExpression, primaryExpression), parse.many(accessor)), (function(expression, props) {
                    return parse.always(stream.foldl(reducer, expression, props));
                })));
            }
        }
    })();
    (newExpression = parse.Parser("New Expression", ecma_parse.nodea(parse.next(token.keyword("new"), parse.enumeration(parse.expected("member expression", memberExpression), parse.optional([], args))), ast_expression.NewExpression.create)));
    var leftHandSideExpression = (function() {
        {
            var reducer = (function(p, c) {
                ...
                return parse.Parser("Left Hand Side Expression", parse.binds(parse.enumeration(parse.memo(memberExpression), parse.many(parse.either(args, accessor))), (function(member, accessors) {
                    return parse.always(stream.foldl(reducer, member, accessors));
                })));
            }
        }
    })();
    var leftHandReferenceExpression = (function() {
        {
            var reducer = (function(p, c) {
                return c.reduce(accessorReducer, p);
            }); {
                return parse.Parser("Left Hand Reference Expression", parse.binds(parse.enumeration(parse.choice(thisExpression, value.identifier), parse.many(accessor)), (function(expression, props) {
                    return parse.always(stream.foldl(reducer, expression, props));
                })));
            }
        }
    ...
    var unaryExpression = (function() {
        {
            var reducer = (function(argument, op) {
                return ast_expression.UnaryExpression.create(position.SourceLocation.merge(op.loc, argument.loc), op.value, argument);
            }); {
                return parse.Parser("Unary Expression", parse.binds(parse.enumeration(parse.many(unaryOperator), parse.expected("left hand side expression", leftHandSideExpression)), (function(ops, expression) {
    ...
    (conditionalExpression = (function() {
        {
            var conditionalBody = parse.next(token.punctuator("?"), parse.enumeration(parse.expected("expression", expression), token.punctuator(":"), parse.expected("expression", expression))); {
                return parse.bind(binaryExpression, (function(bin) {
                    return parse.optional(bin, ecma_parse.nodea(conditionalBody, (function(loc, consequent, _1, alternate) {
    ...
    var assignmentExpression = parse.Parser("Assignment Expression", ecma_parse.nodea(parse.enumeration(leftHandReferenceExpression, assignmentOperator, parse.expected("expression", expression)), (function(loc, left, op, right) {
