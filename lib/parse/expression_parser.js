 ...
 * DO NOT EDIT
*/
define(["require", "parse/parse", "parse/parse_eager", "nu/stream", "ecma/parse/common", "ecma/position", "khepri/parse/program_parser", "khepri/parse/value_parser", "ecma/parse/token_parser", "ecma/ast/node", "ecma/ast/expression", "ecma/ast/statement", "ecma/ast/value"], function(require, parse, parse_eager, stream, ecma_parse, position, program, value, token, node, astExpression, astStatement, astValue) {
    "use strict";
    ...
    var arrayElements = parse.Parser("Array Elements", parse.bind(parse.sepBy(token.punctuator(","), parse.expected("array element", arrayElement)), function(elements) {
        return parse.always(stream.toArray(elements));
    }
    ));
    var arrayLiteral = parse.Parser("Array Literal", ecma_parse.node(parse.between(token.punctuator("["), token.punctuator("]"), arrayElements), function(loc, elements) {
    ...
    var propertyGetInitializer = parse.Parser("Property Get Initializer", ecma_parse.nodea(parse.next(token.keyword("get"), parse.sequence(propertyName, token.punctuator("("), token.punctuator(")"), parse.between(token.punctuator("{"), token.punctuator("}"), functionBody))), function(loc, name, _0, _1, body) {
        return ({
            "loc": loc,
            "key": name,
            "kind": "get",
    ...
    var propertySetInitializer = parse.Parser("Property Set Initializer", ecma_parse.nodea(parse.next(token.keyword("set"), parse.sequence(propertyName, parse.between(token.punctuator("("), token.punctuator(")"), propertySetParameterList), parse.between(token.punctuator("{"), token.punctuator("}"), functionBody))), function(loc, name, parameters, body) {
        return ({
            "loc": loc,
            "key": name,
            "kind": "set",
    ...
    var objectProperties = parse.Parser("Object Properties", parse.bind(parse.sepBy(token.punctuator(","), propertyInitializer), function(props) {
        return parse.always(stream.toArray(props));
    }
    ));
    var objectLiteral = parse.Parser("Object Literal", ecma_parse.node(parse.between(token.punctuator("{"), token.punctuator("}"), objectProperties), function(loc, properties) {
    ...
    var ecmaFunctionExpression = ecma_parse.nodea(parse.next(token.keyword("function"), parse.sequence(parse.optional(value.identifier), parse.between(token.punctuator("("), token.punctuator(")"), formalParameterList), parse.between(token.punctuator("{"), token.punctuator("}"), functionBody))), function(loc, id, parameters, body) {
    ...
    var lambdaBody = parse.either(parse.between(token.punctuator("{"), token.punctuator("}"), functionBody), ecma_parse.node(expression, function(loc, x) {
    ...
    var primaryExpression = parse.Parser("Primary Expression", parse.choice(thisExpression, parse.attempt(parse.bind(parse.either(value.identifier, parse.between(token.punctuator("("), token.punctuator(")"), expression)), function(x) {
    ...
    var argumentList = parse.Parser("Argument List", parse_eager.sepBy(token.punctuator(","), parse.expected("assignment expression", assignmentExpression)));
    var args = parse.Parser("Arguments", ecma_parse.node(parse.between(token.punctuator("("), token.punctuator(")"), argumentList), function(loc, args) {
        (args.loc = loc);
        return args;
    }
    ));
    ...
    var bracketAccessor = parse.Parser("Bracket Accessor", parse.bind(parse.between(token.punctuator("["), token.punctuator("]"), parse_eager.sepBy1(token.punctuator(","), expression)), function(accessor) {
    ...
    var memberExpression = function() {
        ...
        ;
        return parse.Parser("Member Expression", parse.binda(parse.sequence(parse.either(newExpression, primaryExpression), parse.many(accessor)), function(expression, props) {
    ...
    var leftHandSideExpression = function() {
        var reducer = function(p, c) {
            return (((Array.isArray(c) && (c.length > 0)) && c[0].hasOwnProperty("property")) ? c.reduce(accessorReducer, p) : new astExpression.CallExpression(position.SourceLocation.merge(p.loc, c.loc), p, c));
        }
        ;
        return parse.Parser("Left Hand Side Expression", parse.binda(parse.sequence(parse.memo(memberExpression), parse.many(parse.either(args, accessor))), function(member, accessors) {
    ...
    var unaryExpression = function() {
        ...
        ;
        return parse.Parser("Unary Expression", parse.binda(parse.sequence(parse.many(unaryOperator), parse.expected("left hand side expression", leftHandSideExpression)), function(ops, expression) {
    ...
    var letExpression = parse.RecParser("Let Expression", function(self) {
        return parse.either(ecma_parse.nodea(parse.next(token.keyword("let"), parse.sequence(value.identifier, token.punctuator("="), conditionalExpression, token.keyword("in"), self)), function(loc, id, _1, val, _2, expr) {
            return new astExpression.CallExpression(loc, new astExpression.FunctionExpression(null, null, [id], new astStatement.BlockStatement(null, [new astStatement.ReturnStatement(expr)])), [val]);
        }
    ...
    (assignmentExpression = parse.RecParser("Assignment Expression", function(self) {
        return parse.backtrack(parse.either(parse.binda(parse.attempt(parse.sequence(parse.memo(leftHandSideExpression), assignmentOperator)), function(left, operator) {
