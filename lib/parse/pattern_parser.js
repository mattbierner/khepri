...
*/
;
define(["parse/parse", "parse/parse_lang", "ecma/parse/common", "ecma/parse/token_parser", "khepri_ast/pattern", "khepri/parse/value_parser"], function(parse, parse_lang, ecma, token, ast_pattern, value) {
    "use strict";
    ...
    var objectKeyPattern = parse.binds(parse.sequence(parse_lang.then(value.stringLiteral, token.punctuator(":")), pattern), function(key, value) {
        return parse.always(({
            "key": key,
            "value": value
        }));
    ...
    var objectPattern = parse.Parser("Object Pattern", ecma.nodea(parse.sequence(parse.optional(null, identifier), parse_lang.between(token.punctuator("{"), token.punctuator("}"), parse.eager(parse_lang.sepBy1(token.punctuator(","), objectKeyPattern)))), ast_pattern.ObjectPattern.create));
