...
define(["require", "exports", "parse/parse", "parse/lang", "khepri_ast/pattern", "khepri/parse/common", "khepri/parse/token_parser", "khepri/parse/value_parser"], (function(require, exports, __o, __o0, ast_pattern, __o1, __o2, __o3) {
    "use strict";
    var pattern, identifierPattern, ellipsisPattern, importPattern, arrayPattern, objectPattern, argumentsPattern;
    var __o = __o,
        always = __o["always"],
        attempt = __o["attempt"],
        bind = __o["bind"],
        ...
        stringLiteral = __o3["stringLiteral"]; {
            ...
            (importPattern = Parser("Import Pattern", next(keyword("import"), nodea(enumeration(stringLiteral, either(attempt(objectPattern), identifierPattern)), ast_pattern.ImportPattern.create))));
            (argumentsPattern = Parser("Arguments Pattern", (function() {
                {
                    var argumentsPatternElement = choice(ellipsisPattern, attempt(arrayPattern), attempt(objectPattern), identifierPattern); {
                        return either(attempt(nodea(enumeration(optional(null, identifierPattern), between(punctuator("("), punctuator(")"), eager(sepBy(punctuator(","), argumentsPatternElement)))), ast_pattern.ArgumentsPattern.create)), node(eager(sepBy(punctuator(","), argumentsPatternElement)), (function(loc, elements) {
                            return ast_pattern.ArgumentsPattern.create(loc, null, elements);
                        })));
                    }
                }
            })()));
            (pattern = Parser("Pattern", choice(ellipsisPattern, importPattern, attempt(arrayPattern), attempt(objectPattern), identifierPattern)));
    }
