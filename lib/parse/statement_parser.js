...
define(["parse/parse", "parse/parse_eager", "ecma/parse/statement_parser", "ecma/parse/token_parser", "khepri/parse/_common", "khepri/parse/expression_parser", "khepri/parse/value_parser", "ecma/ast/clause", "ecma/ast/declaration", "ecma/ast/statement"], function(parse, parse_eager, ecma_statement, token, ecma_parse, expression, value, astClause, astDeclaration, astStatement) {
    ...
    var continueStatement = parse.Parser("Continue Statement", ecma_parse.astNode(parse.next(token.keyword("continue"), parse.bind(logicalSemiColon, function() {
    ...
    var caseClause = parse.next(token.keyword("case"), parse.binda(parse.sequence(expression.expression, token.punctuator(":"), statementList), function(test, _, consequent) {
        return parse.always(new astClause.SwitchCase(test, consequent));
    }
    ));
    var defaultClause = parse.next(token.keyword("default"), parse.next(token.punctuator(":"), parse.bind(statementList, function(consequent) {
    ...
    (statement = parse.Parser("Statement", parse.choice(blockStatement, variableStatement, emptyStatement, ifStatement, iterationStatement, continueStatement, breakStatement, returnStatement, switchStatement, throwStatement, tryStatement, debuggerStatement, parse.next(parse.always(null), expressionStatement))));
    return ({
