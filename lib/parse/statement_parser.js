 ...
 * DO NOT EDIT
*/
define(["require", "exports", "parse/parse", "parse/lang", "ecma/parse/token_parser", "ecma/parse/common", "khepri_ast/clause", "khepri_ast/declaration", "khepri_ast/statement", "khepri/parse/expression_parser", "khepri/parse/pattern_parser", "khepri/parse/value_parser"], (function(require, exports, parse, __o, __o0, __o1, ast_clause, ast_declaration, ast_statement, __o2, __o3, __o4) {
    "use strict";
    ...
    var parse = parse,
        ...
        identifier = __o4["identifier"]; {
            ...
            (staticStatement = Parser("Static Statement", node(between(keyword("static"), logicalSemiColon, staticDeclarationList), ast_declaration.StaticDeclaration.create)));
            var initialiser = next(punctuator("="), topLevelExpression);
            var variableDeclaration = nodea(enumeration(identifier, optional(null, initialiser)), ast_declaration.VariableDeclarator.create);
            ...
            var withBinding = parse.Parser("With Binding", either(importPattern, nodea(enumeration(then(withIdentifier, punctuator("=")), expression), ast_declaration.Binding.create)));
            (withStatement = Parser("With Statement", nodea(next(keyword("with"), enumeration(eager(sepBy1(punctuator(","), withBinding)), next(keyword("in"), blockStatement))), ast_statement.WithStatement.create)));
            ...
            var forStatement = Parser("For Statement", nodea(next(keyword("for"), enumeration(next(punctuator("("), forInitExpression), next(punctuator(";"), forTestExpression), next(punctuator(";"), forUpdateExpression), next(punctuator(")"), statement))), ast_statement.ForStatement.create));
            ...
            (switchStatement = Parser("Switch Statement", nodea(next(keyword("switch"), enumeration(between(punctuator("("), punctuator(")"), topLevelExpression), caseBlock)), ast_statement.SwitchStatement.create)));
            ...
            (tryStatement = Parser("Try Statement", nodea(next(keyword("try"), enumeration(blockStatement, optional(null, catchBlock), optional(null, finallyBlock))), ast_statement.TryStatement.create)));
