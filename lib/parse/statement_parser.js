...
*/
;
define(["parse/parse", "parse/parse_lang", "ecma/parse/token_parser", "ecma/parse/common", "khepri_ast/clause", "khepri_ast/declaration", "khepri_ast/statement", "khepri/parse/expression_parser", "khepri/parse/value_parser"], function(parse, parse_lang, token, ecma_parse, ast_clause, ast_declaration, ast_statement, expression, value) {
    "use strict";
    ...
    var blockStatement = parse.Parser("Block Statement", ecma_parse.node(parse_lang.between(token.punctuator("{"), token.punctuator("}"), statementList), function(loc, body) {
        return new ast_statement.BlockStatement(loc, body);
    }
    ));
    var staticDeclaration = ecma_parse.node(value.identifier, function(loc, identifier) {
        return new ast_declaration.StaticDeclarator(loc, identifier, null);
    }
    );
    ...
    var staticStatement = parse.Parser("Static Statement", ecma_parse.node(parse_lang.between(token.keyword("static"), logicalSemiColon, staticDeclarationList), function(loc, declarations) {
        return new ast_declaration.StaticDeclaration(loc, declarations);
    }
    ));
    var initialiser = parse.next(token.punctuator("="), topLevelExpression);
    var variableDeclaration = ecma_parse.nodea(parse.sequence(value.identifier, parse.optional(null, initialiser)), ast_declaration.VariableDeclarator.create);
    ...
    var ifStatement = parse.Parser("If Statement", ecma_parse.nodea(parse.next(token.keyword("if"), parse.sequence(parse_lang.between(token.punctuator("("), token.punctuator(")"), topLevelExpression), statement, parse.optional(null, parse.next(token.keyword("else"), statement)))), ast_statement.IfStatement.create));
    var whileStatement = parse.Parser("While Statement", ecma_parse.nodea(parse.next(token.keyword("while"), parse.sequence(parse_lang.between(token.punctuator("("), token.punctuator(")"), topLevelExpression), statement)), ast_statement.WhileStatement.create));
    ...
    var caseClause = ecma_parse.nodea(parse.next(token.keyword("case"), parse.sequence(parse_lang.then(topLevelExpression, token.punctuator(":")), statementList)), ast_clause.SwitchCase.create);
    var defaultClause = ecma_parse.node(parse.next(token.keyword("default"), parse.next(token.punctuator(":"), statementList)), function(loc, consequent) {
        return new ast_clause.SwitchCase(loc, null, consequent);
    }
    );
    ...
    var switchStatement = parse.Parser("Switch Statement", ecma_parse.nodea(parse.next(token.keyword("switch"), parse.sequence(parse_lang.between(token.punctuator("("), token.punctuator(")"), topLevelExpression), caseBlock)), ast_statement.SwitchStatement.create));
    ...
    var catchBlock = ecma_parse.nodea(parse.next(token.keyword("catch"), parse.sequence(parse_lang.between(token.punctuator("("), token.punctuator(")"), value.identifier), blockStatement)), ast_clause.CatchClause.create);
    var finallyBlock = parse.next(token.keyword("finally"), blockStatement);
    var tryStatement = parse.Parser("Try Statement", ecma_parse.nodea(parse.next(token.keyword("try"), parse.sequence(blockStatement, parse.optional(null, catchBlock), parse.optional(null, finallyBlock))), ast_statement.TryStatement.create));
